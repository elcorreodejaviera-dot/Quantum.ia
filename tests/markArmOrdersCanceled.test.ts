...
describe("markArmOrdersCanceled (JAV-96)", () => {
  it("open/pending → canceled; filled NO cambia; solo los cloids pasados", async () => {
    const t = makeConvexTest();
    const { armId, oOpen, oPending, oFilled, oOther } = await t.run(async (ctx) => {
      ...
      const oOther = await seedOrder(ctx, armId, "0xother", { role: "tp_final", observedStatus: "open" }); // NO en la lista
      return { armId, oOpen, oPending, oFilled, oOther };
    });
    const r = await t.mutation(internal.triggerArms.markArmOrdersCanceled, {
      armId, token: "owner", cloids: ["0xopen", "0xpending", "0xfilled"],
    });
    expect(r.ok).toBe(true);
    expect(r.n).toBe(2);                                  // solo open + pending
    expect(await obs(t, oOpen)).toBe("canceled");
    expect(await obs(t, oPending)).toBe("canceled");
    expect(await obs(t, oFilled)).toBe("filled");          // terminal real intacto
